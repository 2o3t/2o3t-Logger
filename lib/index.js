...
class Logger {
    ...
    constructor(name, options = {}) {
        ...
        this[INIT_DIR](options);
        this[PARAM_CONFIG] = defaultConfig(options);
        this[INIT]();
        this[INIT_READY]();
    ...
    _injectName(args) {
        ...
        if (this[GLOBAL_NAME] && args[0] !== this[GLOBAL_NAME]) {
    ...
    [INIT_DIR](options) {
        const bOpenFile = isOpenFile(options);
        // 配置日志相关
        if (bOpenFile) {
            initLoggerDir();
        }
    ...
    [INIT_READY]() {
        ...
        Logger.methods.forEach(name => {
            ...
            this[name] && (this[name].hook = function() {
                if (_that.disabled(name)) return;
                const args = Array.prototype.slice.apply(arguments);
                _that[name].apply(_that, args);

                // hook 出去
                if (Logger.hooks && (utility.is.function(Logger.hooks))) {
                    Logger.hooks(name, args);
                }
            });
